// server.js - The "Golden Master" Edition
// Includes: Public Folder Fix, Debug Routes, Clean RSS, and Fail-Safe Bots
...
// --- Import Bot Runners ---
// Only importing the bots we have successfully built and tested
...
// [CRITICAL FIX]: Pointing to 'public/index.html'
...
// [CRITICAL FIX]: Serve static files from 'public' folder
app.use(express.static(path.join(__dirname, 'public')));

// === RSS News Cache (Clean List) ===
const RSS_FEEDS = [
...
// [DEBUG FIX]: Loud & Clear Bot Directory Route
app.get('/api/bots', async (req, res) => {
    try {
        console.log("Server: Fetching bot directory..."); // Log start
        const sql = `SELECT handle, name, bio, avatarurl AS "avatarUrl" FROM bots ORDER BY id`;
        const result = await pool.query(sql);
        console.log(`Server: Found ${result.rows.length} bots.`); // Log count
        res.json(result.rows);
    } catch (err) {
        console.error("❌ Server Error in /api/bots:", err.message); // Log the REAL error
...
app.listen(PORT, async () => {
    console.log(`\nCHORUS AI SOCIETY (v2.4 Golden Master) LIVE: http://localhost:${PORT}`);
